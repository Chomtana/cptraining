/*
Crit case found : ending case -> qs[s+l-1] >= n

High resolution time complexity:
A(4n + (n/(40log(l)))*log(n))

Low resolution time complexity:
A(n)
because (n/(40log(l)))*log(n) approx n (lower than n for large l)
*/
...
pii query(int ts,int te,int treei,int s,int e) {
    if (e<ts || s>te) return mp(-1000000000000000L,-1);
...
void build(int treei,int s,int e) {
    if (s==e) {
        tree[treei] = qs[s];
    } else {
        int mid = (s+e)/2;
        build(2*treei,s,mid);
        build(2*treei+1,mid+1,e);
...
int main() {
    ...
    pii maxval = query(0,l-1);
    bool needquery = false;

    for1(s,0,n) {
        if (needquery) {
            if (s+l<=n) {
                maxval = query(s,s+l-1);
            } else {
                maxval = query(s,n-1);
            }
        } else {
            //make b,c,d -> b,c,d,e
            if (s+l<=n) {
                if (qs[s+l-1].first>maxval.first) {
                    maxval.first = qs[s+l-1].first;
                    maxval.second = s+l-1;
                }
            }
        }
        ...
        //make a,b,c,d -> b,c,d
        //if max pos != a -> max(b,c,d) = max(a,b,c,d)
        //as same for other problem ... if max pos != c -> max(a,b,d) = max(a,b,c,d)
        //if max pos == a -> cannot conclude
        if (maxval.second == s) {
            needquery = true;
        } else {
            needquery = false;
