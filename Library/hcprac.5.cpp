      ...
      if (area > maxsize) {
        if (previousok) {
          if (rand() % 100 < rand() % 25) {
            j--;
            if (j<i+1) j=i+1;
            if (Tcount.sum(sr,i,er,j-1) >= mininb && Mcount.sum(sr,i,er,j-1) >= mininb) {

            } else {
              j++;
            }
          } else if (rand() % 100 < rand() % 25) {
            j-=2;
            if (j<i+1) j=i+1;
            if (Tcount.sum(sr,i,er,j-1) >= mininb && Mcount.sum(sr,i,er,j-1) >= mininb) {
          ...
          totalscore += (er-sr+1)*(j-i);
          res.push_back(vi{sr,i,er,j-1});
          i = j;
          i--;
        }
        break;
    ...
    for1(j,i,nr) {
      ...
      if (curr >= currmax) {
        ...
        if (rand() % 100 < rand() % 25) {
          ...
          if (maxj<i) maxj=i;
          subsubans.clear();
          ll newcurr = solveCol(i,maxj,subsubans);
          if (newcurr > 0) {
            currmax = newcurr;
            subans = subsubans;
          }
