...
int recur(int cr,int cc,int use) {
    if (cr>=nr||cc>=nc) return 0;
    if (dist[cr][cc][use]!=-1) return dist[cr][cc][use];
    int res = 0;
    if (use<2) {
        res = max(max(recur(cr+1,cc,use+1)+data[cr][cc],recur(cr,cc+1,use+1)+data[cr][cc]),max(recur(cr+1,cc,0),recur(cr,cc+1,0)));
    } else {
        res = max(recur(cr+1,cc,0),recur(cr,cc+1,0));
    }
    return dist[cr][cc][use]=res;
}
