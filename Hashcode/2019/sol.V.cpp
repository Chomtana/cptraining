...
pii solveOneV(int a,int b) {
  ...
  foreach(cc,avail) {
    int c = cc.second;
    if (intag[b].size() + cc.first < currmax) break;
    if (!isH[c]) {
      if (c==a || c==b) continue;
...
ll solveOneSlideOld() {
  ...
  if (maxa != maxb) {
    if (isH[maxb]) {
      ...
      avail.erase(avail.find({intag[maxc].size(),maxc}));
...
int main() {
  ...
  if (isH[begini]) {
    ...
    avail.erase(avail.find({intag[begini].size(),begini}));
    avail.erase(avail.find({intag[begini2].size(),begini2}));
  }
