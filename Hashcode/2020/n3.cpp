...
intonly main() {
  ...
  while (true) {
    ...
    sort(all(signupOrder), [mysw, mysw2](auto a, auto b) {
      // if (lib[a].signup == lib[b].signup)
      // {
      // return lib[a].ship < lib[b].ship;
      // }
      return (double)lib[a].signup/mysw2 - mysw * ((double)lib[a].bookn / (double)lib[a].ship) < (double)lib[b].signup/mysw2 - mysw * ((double)lib[b].bookn / (double)lib[b].ship);
    });
