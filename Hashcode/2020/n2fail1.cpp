...
class Lib {
  public:
    ...
    int scanBook(int bookid) {
      if (currday >= days) {
        if (scan_books.size() == 0) return currday;

        bookqueue.push_back(books.back());
        scan_books.pop_back();
        round_scan_books--;
      }
...
intonly main() {
  ...
  sort(all(signupOrder), [] (auto a, auto b) {
    return lib[a].signup < lib[b].signup;
  });
  ...
  for1(i,0,n) bookqueue.push_back(i);

  int ITE_COUNT = 1;

  while (ITE_COUNT--) {
    deque<int> booki_byrare = bookqueue;
